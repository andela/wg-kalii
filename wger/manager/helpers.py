...
MICROCYCLE, MESOCYCLE, MACROCYCLE = 'microcycle', 'mesocycle', 'macrocycle'

PERIODIZATION = collections.namedtuple('PERIODIZATION', [MICROCYCLE, MESOCYCLE,
                                                         MACROCYCLE])
...
class Periodization(object):
    """
    Contains helper methods that return maximum and minimum number of minths in
    a selected cycle
    """
    def __init__(self):
    ...
    def get_min(self, cycle):
        """
        Return minimum number of months for specified periodization cycle
        """
    ...
    def translate(self, duration):
        ...
        """
        if (duration > self.get_max(MACROCYCLE) or
                duration < self.get_max(MICROCYCLE)):
                    return '%s Week(s)' % duration

        if duration == 1:
            return 'Microcycle (1 week)'

        if duration in range(self.get_min(MESOCYCLE),
                             (self.get_max(MESOCYCLE) + 1)):
                                return 'Mesocycle (2-6 Weeks)'

        if duration in range(self.get_min(MACROCYCLE),
                             (self.get_max(MACROCYCLE) + 1)):
